...
\section{Function}
	\subsection{Functions}
		\subsubsection{Web Application Interface} \label{def:webInt}
		...
		\subsubsection{ZIP code submission} \label{def:zip}
		...
		\subsubsection{Upload Photo} \label{def:upload}
		...
		\subsubsection{Extract EXIF} \label{def:extexif}
		...
		\subsubsection{Call Weather API} \label{def:weathAPI}
			...
			Precondition: The user submitted their ZIP code as per use case ZIP Code Submission \ref{def:zip} or The user submitted an image that was successfully identified as per use case Classify Photo \ref{def:classPh}.\\
			Basic Path:
			\begin{enumerate}
		...
		\subsubsection{Identify Aerosols} \label{def:idAero}
